...
const newMapping = [];
users.map((user) => {
  let obj = { email: user.email, companyName: user.company.name };
  newMapping.push(obj);
});
...
newArr = users.filter((user) => {
  return user.address.zipcode.charAt(0) === "5";
});
console.log(newArr);
...
newArr3 = users
  ...
  .filter((user) => {
    return user.charAt(0) === "C";
  });
console.log(newArr3);

//Ex5
const onlyFromSChristy =
  users.filter((user) => {
...
console.log(onlyFromSChristy);

//Ex6
const apt950 = users.find((user) => {
  return user.address.suite === "Apt. 950";
});
console.log(apt950.company.name);

//Ex7
const story = function (user) {
  console.log(
...
const studentGrades = studentScores.map((student) => {
  if (student >= 90) {
    return "A";
  } else if (student >= 80) {
    return "B";
...
const grades = studentGrades.reduce(
  (obj, grade) => {
    obj[grade] = obj[grade] + 1;
    return obj;
  },
  { A: 0, B: 0, C: 0, F: 0 }
);
...
const sumItems = cartItems.reduce((acc, item) => {
  ...
  const calc = acc + price;
  if (item.category === "clothing") {
    return calc + price * taxRates.clothing;
  } else if (item.category === "electronics") {
    return calc + price * taxRates.electronics;
  } else {
    return calc + price * taxRates.food;
  }
}, 0);
